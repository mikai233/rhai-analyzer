...
fn hover_supports_map_property_topics() {
    ...
    assert!(
        docs.contains("[\"name\"]"),
        "expected property docs to mention bracket syntax"
    );
    assert!(
        hover
            .notes
            .iter()
            .any(|note| note.contains("map[\"field\"]")),
        "expected property hover notes to mention bracket form"
    );
}

#[test]
fn hover_supports_map_index_topics_with_property_cross_reference() {
    let mut host = AnalysisHost::default();
    host.apply_change(ChangeSet::single_file(
        "main.rhai",
        r#"
    ...
    let hover = analysis
        .hover(FilePosition { file_id, offset })
        .expect("expected map index hover");
    ...
    assert!(
        docs.contains(".name"),
        "expected index docs to mention property syntax"
    );
    assert!(
        hover.notes.iter().any(|note| note.contains("map.field")),
